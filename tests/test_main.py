...
import main
from main import load_config, _parse_email, _write_transaction, run_undo, run_gmail
...
class TestWriteTransaction:
    def test_success(self, config, monkeypatch):
        monkeypatch.setattr("main.category_store.save", lambda *a: None)
        monkeypatch.setattr("main.merchant_store.learn", lambda *a: None)
    ...
    def test_updates_categories(self, config, monkeypatch):
        saved = {}
        monkeypatch.setattr("main.category_store.save", lambda m, *a: saved.update(m))
        monkeypatch.setattr("main.merchant_store.learn", lambda *a: None)
        categories = {}
    ...
    def test_marks_processed(self, config, monkeypatch):
        monkeypatch.setattr("main.category_store.save", lambda *a: None)
        monkeypatch.setattr("main.merchant_store.learn", lambda *a: None)
        monkeypatch.setattr("main.dedup_store.mark_processed", lambda ids, mid: ids.add(mid))
        processed = set()
        txn = {"date": "02/21/2026", "item": "Test", "category": "Misc", "amount": 5.25}
    ...
    def test_no_dedup_without_msg_id(self, config, monkeypatch):
        ...
        monkeypatch.setattr("main.merchant_store.learn", lambda *a: None)
        mark_called = []
        monkeypatch.setattr("main.dedup_store.mark_processed", lambda *a: mark_called.append(1))
        txn = {"date": "02/21/2026", "item": "Test", "category": "Misc", "amount": 5.25}
...
class TestRunGmail:
    def _setup_gmail_mocks(self, monkeypatch, emails, parse_result=None):
        ...
        # Inject mock into sys.modules so `import gmail_reader` inside run_gmail gets it
        monkeypatch.setitem(sys.modules, "gmail_reader", mock_gmail)
        # Also set on main module in case it was already imported
        monkeypatch.setattr(main, "gmail_reader", mock_gmail, raising=False)
        monkeypatch.setattr("main.dedup_store.load", lambda: set())
        monkeypatch.setattr("main.dedup_store.mark_processed", lambda ids, mid: ids.add(mid))
        monkeypatch.setattr("main.category_store.save", lambda *a: None)
        monkeypatch.setattr("main.merchant_store.learn", lambda *a: None)
        return mock_gmail
    ...
    def test_all_write(self, config, monkeypatch, sample_capitalone_email):
        ...
        monkeypatch.setattr("main.display.prompt_batch_action", lambda n: ("all", None))
        run_gmail(config, {}, {})
    ...
    def test_skip_some(self, config, monkeypatch, sample_capitalone_email):
        ...
        monkeypatch.setattr("main.display.prompt_batch_action", mock_batch_action)
        run_gmail(config, {}, {})
    ...
    def test_dedup_skips(self, config, monkeypatch, sample_capitalone_email):
        emails = [("msg1", "Subject", sample_capitalone_email)]
        mock_gmail = self._setup_gmail_mocks(monkeypatch, emails)
        monkeypatch.setattr("main.dedup_store.load", lambda: {"msg1"})
        run_gmail(config, {}, {})
        mock_gmail.mark_as_read.assert_not_called()
    ...
    def test_edit_then_write(self, config, monkeypatch, sample_capitalone_email):
        ...
        monkeypatch.setattr("main.display.prompt_batch_action", mock_batch_action)
        monkeypatch.setattr("main.display.prompt_edit", mock_prompt_edit)
        run_gmail(config, {}, {})
